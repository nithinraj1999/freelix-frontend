...
const ProfileHeader: React.FC<ProfileHeaderProps> = ({ freelancerData }) => {
  ...
  const handleSave = async (newValue: any) => {
    if (currentField) {
      ...
      if (typeof newValue === "string") {
        ...
        setCurrentData((prev) => ({ ...prev, profilePicture: URL.createObjectURL(newValue) }));
        formData.append("profilePicture", newValue);
      }
  ...
  const getInputType = (field: keyof typeof currentData): "text" | "textarea" | "file" => {
    return field === "profilePicture" ? "file" : "text";
  };
  ...
  <img
    src={currentData.profilePicture}
    alt="Profile"
    className="absolute top-[-100px] left-32 transform -translate-x-1/2 border-4 border-white rounded-full w-40 h-40 object-cover"
  />
  {/* {user?.role === "freelancer" && (
    <button
      onClick={() => handleEdit("profilePicture")}
      className="absolute top-[-100px] left-[185px] items-center bg-gray-200 p-2 rounded-full w-8 h-8 hover:bg-gray-300 transition duration-200"
    >
      ✎
    </button>
  )} */}
