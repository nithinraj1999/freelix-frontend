...
const JobList: React.FC = () => {
  ...
  return (
    <div className="bg-green-50 w-full mt-32">
      <div className="h-[230px] bg-teal-500 px-8 py-8 rounded">
      ...
      {jobList.length > 0 ? (
        jobList.map((job) => (
          <div
            key={job._id}
            className="w-full mt-4 bg-black rounded px-8 py-8 min-h-[250px] cursor-pointer"
          >
            <div className="flex justify-between items-start">
              <h1 className="text-white text-xl font-bold">
                {job.title}
              </h1>
              <IoBookmarkOutline color="white" size={25} />
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {job.skills.map((skill, index) => (
                <span key={index} className="bg-slate-100 text-black rounded-full px-3 py-1 text-sm">
                  {skill}
                </span>
              ))}
            </div>
